...
class Model(nn.Module):
    def __init__(self, c1=16, c2=24, blocks=('t1', 'x', 't2', 't4', 'x', 't8'), k_onset=K_ONSET):
        super().__init__()
        self.cfg = dict(c1=c1, c2=c2, blocks=list(blocks), k_onset=k_onset)
...
def macs(model):
    c = model.cfg
    c1, c2 = c['c1'], c['c2']
    n = 2 * len(HARM) * c1 * NPOS + 9 * c1 * NPOS + 3 * c1 * c2 * NKEY
